...
export const TvLobby: React.FC<TvLobbyProps> = ({
  ...
  const teams: { key: TeamColor; label: string; bg: string; border: string; text: string }[] = [
    { key: 'red', label: t.teams.red, bg: 'bg-rose-500/10', border: 'border-rose-500/30', text: 'text-rose-400' },
    { key: 'green', label: t.teams.green, bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', text: 'text-emerald-400' },
    { key: 'blue', label: t.teams.blue, bg: 'bg-sky-500/10', border: 'border-sky-500/30', text: 'text-sky-400' },
    { key: 'yellow', label: t.teams.yellow, bg: 'bg-amber-500/10', border: 'border-amber-500/30', text: 'text-amber-400' },
  ];
  ...
  return (
    <div className="w-full flex flex-col space-y-6 animate-in fade-in duration-300" dir={lang === 'dz' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h2 className="text-3xl font-black text-white tracking-tight">{roomInfo.name}</h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div
            ...
            tabIndex={0}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-slate-950 border transition ${
          ...
          <button
            ...
            className={`px-8 py-3.5 rounded-2xl font-black text-base shadow-xl transition flex items-center gap-2 cursor-pointer active:scale-95 ${
              players.length > 0
                ? focusedIndex === 0
          ...
          <button
            ...
            className={`px-4 py-3 rounded-2xl bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 border border-rose-900/40 text-xs font-bold transition cursor-pointer ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {teams.map((team) => {
          ...
          return (
            <div
              key={team.key}
              className={`bg-slate-900 border ${team.border} rounded-3xl p-5 shadow-lg flex flex-col justify-between min-h-[340px] relative overflow-hidden`}
            >
              {/* Team Header */}
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className={`font-black text-base tracking-wide ${team.text}`}>{team.label}</h3>
                  </div>
                    ...
                    teamPlayers.map((p, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <div
                            className={`w-8 h-8 rounded-xl ${team.bg} border ${team.border} flex items-center justify-center font-bold text-xs ${team.text}`}
                          >
                            #{idx + 1}
                          </div>
                          <div>
                            <div className="font-bold text-sm text-white">{p.name}</div>
                            <div className="text-[10px] text-slate-500 font-mono">{p.characterId}</div>
                          </div>
                        </div>
      ...
      {players.length === 0 && (
        <div className="bg-slate-900 border border-dashed border-slate-800 rounded-3xl p-8 text-center space-y-3">
          <Radio className="w-8 h-8 text-sky-400 mx-auto animate-pulse" />
          <h4 className="text-base font-bold text-slate-200">{t.waitingForPlayers}</h4>
